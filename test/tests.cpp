...
static bool contains(const std::vector<std::string>& vec, const std::string& word) {
...
TEST(GenerateTextTest, MultipleSuffixes_ResultInSet) {
    ...
    std::vector<std::string> all_suffixes = { "c", "d", "e", "X", "Y", "Z", "W" };
    for (size_t i = 2; i < words.size(); ++i) {
        EXPECT_TRUE(contains(all_suffixes, words[i])) << "word \"" << words[i] << "\" not in set";
    }
}
...
TEST(GenerateTextTest, FirstWordsEqualFirstPrefix) {
    ...
    EXPECT_TRUE(result.substr(0, 7) == "one two");
}